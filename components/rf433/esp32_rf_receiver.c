...
static unsigned nSeparationLimit = 4300;
static long last_processed_time = 0;
...
void esp_rf433_interrupt_task(void *arg)
{
  ...
  if (duration > nSeparationLimit)
  {
    if (diff(duration, timings[0]) < 200)
    {
      ...
      if (repeatCount == 2)
      {
        // УПРОЩЕННАЯ ЛОГИКА - убираем строгий дебаунсинг на уровне приемника
        for (uint8_t i = 1; i <= numProto; i++)
        {
          if (esp_rf433_receive_protocol(i, changeCount))
          {
            last_processed_time = time;
            last_received_value = nReceivedValue;
